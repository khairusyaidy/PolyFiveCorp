...
public class MyTableModel extends AbstractTableModel {
	...
	private void setVerticalSeatIndices() {
		for (int i = 1; i < this.rows - 1; ++i) { // replace A with B achieved
													// row is letter and column
													// is numbers
			cells[i][0].cellData = cells[i][this.cols - 1].cellData = String
					.valueOf(i);
	...
	private void setSeatPlaces() {
		for (int i = 1; i < this.rows - 1; ++i) {
			for (int j = 1; j < this.cols - 1; j++) {// replace A with B
														// achieved row is
														// letter and column is
														// numbers
				cells[i][j].cellData = cells[i][this.cols - 1].cellData = Character
						.toString((char) (0x40 + i)) + String.valueOf(j);
	...
	public static void setPricing(int firstRow, int firstCol, int secondRow,
			int secondCol, double seatPrice, int available, int id) {
