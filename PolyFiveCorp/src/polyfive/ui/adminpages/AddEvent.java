 ...
 * Purpose : Allows the admin to add event
 * 
...
public class AddEvent extends MasterPanel {
	...
	public AddEvent(MainFrame frame) {
		...
		button.setIcon(new ImageIcon(AddEvent.class
		...
		Back.setBorder(new EtchedBorder(EtchedBorder.LOWERED, Color.DARK_GRAY,
				null));
		...
		comboBox_2.setModel(new DefaultComboBoxModel(new String[] { "2013",
				"2014" }));
		...
		AddEvent.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				 ...
				 * f.getContentPane().removeAll();
				 * f.getContentPane().add(adminAddEvent); f.repaint();
				...
				if ((price.matches(expression))
						&& (noOfTickets.matches(expression2))) {
					int reply = JOptionPane.showConfirmDialog(null,
							"Are you sure you want to add this new event ?");
					if (reply == JOptionPane.YES_OPTION) {
						String eventName = tfEventName.getText();
						...
						int eventNoOfTicketsInt = Integer
								.parseInt(eventNoOfTickets);
					...
					JOptionPane.showMessageDialog(null,
							"Please make sure that you filled in accurately");
